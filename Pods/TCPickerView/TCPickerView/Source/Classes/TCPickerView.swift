...
open class TCPickerView: UIView, UITableViewDataSource, UITableViewDelegate {
    ...
    open var textColor: UIColor = UIColor(red: 191/255, green: 54/255, blue: 12/255, alpha: 1)
 {
        didSet {
            self.titleLabel?.textColor = self.textColor
            self.doneButton?.setTitleColor(self.textColor, for: UIControlState.normal)
            self.closeButton?.setTitleColor(self.textColor, for: UIControlState.normal)
        }
    }
    ...
    fileprivate func setupUI() {
        ...
        titleLabel.addConstraint(NSLayoutConstraint(item: titleLabel,
            ...
            attribute: .height, multiplier: 1.0, constant: 80))
    ...
    fileprivate func updateUI() {
        ...
        self.doneButton?.setTitle("Select", for: .normal)
